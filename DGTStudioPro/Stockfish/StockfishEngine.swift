...
internal actor StockfishEngine {
    ...
    internal func start() async throws {
        guard process == nil else { throw EngineError.alreadyStarted }
    ...
    private func handleStdoutLine(_ line: String) {
        guard let response = UCIProtocol.parse(line) else { return }
    ...
    private func writeLine(_ command: String) throws {
        guard let stdin = stdinHandle else { throw EngineError.notStarted }
        let data = (command + "\n").data(using: .utf8)!
