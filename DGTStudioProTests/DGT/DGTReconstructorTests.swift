...
struct DGTReconstructorTests {
    ...
    /// The captured pawn must actually leave the board. If the attacker lands
    /// on the EP target but the player forgets to lift the captured pawn, the
    /// candidate EP move's `applying(...)` (which removes that pawn) no longer
    /// matches the physical board — so it's a fumble, not a move. This pins the
    /// fact that EP verification goes through the full board comparison, not
    /// just the `(from, to)` endpoints.
    @Test func enPassantWithoutLiftingCapturedPawnIsUnresolved() throws {
        let state = try GameState.parsing("4k3/8/8/3pP3/8/8/8/4K3 w - d6 0 1")
