...
Revision 30 July 2026, sixth of the day (the M5 epilogue). Base named explicitly, per the working agreement: this is the **M5 revision (fifth of 30 July)** plus one carried delta — **the Players editing UITest and D40′, the orphan sweep**. Everything the M5 revision recorded stands except where corrected below. This revision was written by editing the file at its anchors rather than re-emitting it whole; the "complete .md" agreement exists to stop updates arriving as prose describing edits, and with direct file access an edit *is* the artifact, with none of the transcription drift a full re-emission from a synced copy risks.

One decision minted at recording time: **D40′**, chosen by Bera from concrete options before any diff. The epilogue landed in two reviewed batches with ⌘U green after each. No files added or removed; the source count stays 211 (131 app, 79 unit-test, 1 UITest).

The tree stays committed. Base for this delta is `3f785a3` (M5 batch 3), itself on `0a21fc9`.

**The correction this pass, and it is the milestone's whole content.** Writing M5's missing UITest meant reading its surfaces end to end first, and that showed **Delete Player could never be enabled**: `GameRecord.Side` is built from the resolved links, so `PlayerStats.index(of:)` only emits players holding a game, so selection can only be such a player, so `canDelete = recentGames.isEmpty` was always false. "Is in the list" and "is deletable" were exact complements. M5's own manual-check step for it was impossible to perform. D9′'s intent — a manual door for orphans, not a collector — was right; the door was hung where orphans cannot appear. D40′ moves it.

Next free number: **D41′**.
...
D-numbers are sequential and never reused. Next free number: **D41′**.
...
**M5's two sheets are not on this table** — both ship previews (three and two), the store door they drive is suited, and since the epilogue the *flow* is witnessed too: four UITests cover the header controls' hittability, the rename round trip, the merge, and D40′'s sweep. The hittability test stands alone deliberately, because both header controls are borderless inside a `List` section header — the shape M1 proved AX-invisible for the sidebar's + button — so if that finding transfers, one test names it instead of three failing obscurely.
...
- **Whether an inspector section header's borderless controls are AX-visible on macOS is still unknown**, and `test_players_profileHeaderControls_areHittable` is the experiment. M1 proved the *sidebar* header's + button invisible and answered it with a menu-bar command; if the inspector header behaves the same, the rename pencil and actions menu need the same remedy and three UITests come with the answer. Recorded rather than pre-solved: the fix has a precedent and the evidence is one run away.
