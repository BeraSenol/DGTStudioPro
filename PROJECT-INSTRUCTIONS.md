...
Revision 31 July 2026, **third of the day** (M7's `RosterSummary` experiment — the item D43′ left open, closed by running it). Base named explicitly, per the working agreement: this is the second-of-the-day revision (D43′) plus this pass's deltas — **D44′, two source files, one deleted attribute, and a third species of stale claim**. Everything the earlier revisions recorded stands except where corrected below. Written by editing the file at its anchors.

One decision minted at recording time: **D44′**. Smaller than its two predecessors and the same shape underneath: a sentence in this project asserted something checkable, nobody had checked it, and it was wrong.

Base for this delta is **`f64b8d4`**, and **the tree was clean** — the first pass in four to open that way. `git status` still ran first, which is the only reason that sentence can be written at all. Four consecutive passes had found their predecessor's delivery uncommitted; D43′ broke the streak by committing before the write-up rather than after. Recorded because a rule that finally gets obeyed deserves the same note as one that keeps getting broken, and because the mechanism was ordering, not diligence.

**This pass produced code: one attribute deleted, one test added.** `RosterSummary`'s live-projection init carried `@MainActor` on the stated reason that `LiveGame.Roster` inherits `LiveGame`'s isolation by being nested inside it. It does not. A global actor isolates a type's *members*, never the types *nested* in it — SE-0449 shows this exact shape in its own text. The attribute constrained the init for nothing.

**The finding, and it completes a progression.** D42′ generalised the standing rule from "the tree is clean" to **the file is there**; D43′ extended it to **the number is current**; this pass extends it once more, to **the rule is real**. Those are three species of the same claim — a sentence written confidently, about something a single command could settle, that nothing in the ordinary course of work would ever contradict. This one was the best hidden of the three: a path can be missing and a number can be re-measured, but a false statement about the *language* sits inside a green build indefinitely, because the compiler only contradicts a claim that something in the tree actually exercises. Nothing did. Every caller that builds a `Roster` was already `@MainActor` for `LiveGame`'s sake, so the attribute's premise was untestable by construction until this pass made it testable on purpose.

Next free number: **D45′**.
...
D-numbers are sequential and never reused. Next free number: **D45′**.
