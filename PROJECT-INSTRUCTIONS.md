...
Revision 31 July 2026, first of the day (M7's swift-format item, closed by declining it). Base named explicitly, per the working agreement: this is the **sweep revision (seventh of 30 July)** plus this pass's deltas — **D42′, one untracked file, and one false premise corrected**. Everything the sweep revision recorded stands except where corrected below. Written by editing the file at its anchors, the epilogue's precedent: the "complete .md" agreement exists to stop updates arriving as prose describing edits, and with direct file access an edit *is* the artifact, with none of the transcription drift a full re-emission from a synced copy risks.

One decision minted at recording time: **D42′**. It differs from every D-number before it in one way worth flagging: Bera put the posture to my judgement rather than choosing from the options, so the reasoning below is the whole of the justification and carries no "chosen from concrete options" weight. Re-read it on that basis. No sources added or removed; the count stays 211 (131 app, 79 unit-test, 1 UITest).

Base for this delta is **`abf9e0c`** (the sweep). Two changes carry it — untracking `.DS_Store` and this document — and the tree held nothing else uncommitted when the pass began, which was checked rather than assumed.

**This pass is one M7 item, and it produced no code.** M7's four items are independent; swift-format was taken first because it looked like the cheapest. It was not cheap, it was *undecided* — see D42′. The other three (Instruments, warning triage, language-mode 6) all need a build or a board and so need Bera's hands; none has started.

**The finding, and it is a new species of the standing hazard.** The open-items list asserted that `.swift-format` was *committed*. It has never existed — not tracked, not in history, no marks in any source. The existing agreement covers claims that the tree is **clean**; this is a claim that a file is **there**, which is the same class and just as checkable with one command. `git ls-files` answers it as fast as `git status` answers the other. A doc that names a file is making a claim about the filesystem, and the sharpest version of the rule now reads: any sentence in these documents containing a path is checkable, and the ones written most confidently are the ones nobody has checked since.

Next free number: **D43′**.
...
Tree at delivery: **`abf9e0c` (the sweep), plus this pass's two changes** — `.DS_Store` untracked and gitignored, and this document. Checked with `git ls-files` and `git status`, not assumed. 211 sources (131 app, 79 unit-test, 1 UITest).
...
D-numbers are sequential and never reused. Next free number: **D43′**.
