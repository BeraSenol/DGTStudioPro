...
Revision 31 July 2026, **second of the day** (M7's warning triage and its language-mode-6 evaluation — the first closed as a correction, the second closed by *landing* it). Base named explicitly, per the working agreement: this is the first-of-the-day revision (D42′) plus this pass's deltas — **D43′, three source files, six build-setting lines, and a second false premise corrected**. Everything the earlier revisions recorded stands except where corrected below. Written by editing the file at its anchors, the epilogue's precedent.

One decision minted at recording time: **D43′**. Bera chose it from concrete options, so it carries the usual weight — but note the option he chose was deliberately *contingent* ("fix, re-probe, land wherever it comes back clean"), which made the compiler rather than the decision-maker the thing that picked the endpoint. That is a shape worth reusing when the cost of a migration is unknown and one command can measure it. No sources added or removed; the count stays 211 (131 app, 79 unit-test, 1 UITest).

Base for this delta is **`c93f54f`**. The D42′ pass, which the previous revision described as delivered, was in fact still **uncommitted** when this one began — `git status` found it as the first command, exactly as the sweep's agreement says it will, and it is now `fddf608` (mechanical) and `c93f54f` (the decision). That is the third consecutive pass to find its predecessor's delivery sitting in the working tree. The rule is no longer earning new instances; it is earning the observation that *writing the rule down has not once prevented the recurrence*, and the only thing that has ever caught it is running the command.

**This pass produced code, and the roadmap's framing of it was wrong in a useful way.** M7's warning triage was scheduled as a bucket-and-burn over ~295 warnings. There are none: a cold build of all three targets under the project's own settings emits **zero** compiler diagnostics. The item's real content was hiding one bullet below it — the warnings the project had never asked for. Measured, fixed, and landed: the project now builds in **Swift language mode 6**.

**The finding, and it is D42′'s twin.** The open-items list asserted a warning count of 295. It could not be reproduced under any of the three settings measured — 0 as configured, 230 under complete concurrency, 1 error under mode 6 — and its provenance cannot be reconstructed. D42′ generalised the standing rule from "the tree is clean" to "the file is there"; this pass extends it once more, to **the number is current**. A count in these documents is a claim about a build nobody has re-run, and it decays silently: unlike a path, which is either there or not, a stale number stays plausible indefinitely and reads as scheduled work for as long as nobody types the command. Recorded as unattributed rather than as superseded, because claiming 200-odd warnings were fixed along the way would be inventing a history to make the old number true.

Next free number: **D44′**.
...
Tree at delivery: **`c93f54f` (D42′), plus this pass's four changed files** — `UITestSeed`, `LibraryDestination`, the UITest suite, and `project.pbxproj`. Checked with `git ls-files` and `git status`, not assumed. 211 sources (131 app, 79 unit-test, 1 UITest).

**The build is Swift language mode 6 as of this revision (D43′).** Two warnings remain in the whole project, both on `Binding(present:)`, both waived in writing at the declaration with a sunset condition. Everything else is zero: zero errors, zero warnings in the app target's other 130 sources, zero across all 79 unit-test sources, zero in the UITest suite.
...
D-numbers are sequential and never reused. Next free number: **D44′**.
...
- Collection-destination parity, now with shared metrics — and since the sweep, the **columns** detail grids share them too: `PlayersColumnsView` and `RankingsColumnsView` read `CollectionGridMetrics.spacing` / `.inset` rather than restating 16, which is what `LibraryColumnsView`'s card grid already did. Their `.adaptive(minimum: 160, maximum: 200)` sizing stays local by decision — a detail pane beside a group list is not as wide as a whole destination — so the type governs the icons grids' geometry and only the columns grids' gutters. Known parity residue (unscheduled): the two gallery stat grids still differ, and the two galleries answer empty-selection opposite ways.
...
- RosterSummary's @MainActor live-projection init invites its own removal — delete the attribute and build. Worth re-running on any toolchain move. **Still open, and now cheaper and better-posed:** M7 paired this with the language-mode-6 flip, which has landed (D43′), so the experiment is a one-line deletion against a compiler that is finally checking isolation for real. A mode-5 run of it would have proved much less.
