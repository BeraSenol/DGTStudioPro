...
Revision 30 July 2026, seventh of the day (the between-milestone sweep). Base named explicitly, per the working agreement: this is the **M5-epilogue revision (sixth of 30 July)** plus the sweep's deltas — **D41′, one mechanical fold, and three closed open items**. Everything the epilogue revision recorded stands except where corrected below. Written by editing the file at its anchors, the epilogue's precedent: the "complete .md" agreement exists to stop updates arriving as prose describing edits, and with direct file access an edit *is* the artifact, with none of the transcription drift a full re-emission from a synced copy risks.

One decision minted at recording time: **D41′**, chosen by Bera from concrete options before any diff. No files added or removed; the source count stays 211 (131 app, 79 unit-test, 1 UITest).

Base for this delta is **`3f785a3`** (M5 batch 3). Three commits carry the work — the epilogue (`d33abd3`), D41′ (`43311f9`), and the mechanical fold (`40e201d`), which travelled alone — with this document's own revision riding as the fourth. The base is named rather than the tip deliberately: a tip hash written *inside* the commit that creates it is stale before it is read, which is the smaller cousin of the staleness the sweep found in this very paragraph.

**This pass is a sweep, not a milestone**, run under M5's own new agreement that a grep-level conformance pass belongs *between* milestones and not only inside them. Its result is worth recording precisely because most of it is negative: the standing prohibitions are clean, the D27′ beta surface is still empty, all 144 `AccessibilityID` entries are referenced with no raw strings and no residue from D40′'s removal, all 495 private declarations are referenced, and every one of the fifteen `.disabled(…)` sites survives D40′'s new grep — for each, the enabling value is producible. The two findings that were real are recorded below; the third thing it produced was **evidence**, from the ⌘U run rather than from reading.

**The evidence, and it closes a two-milestone question.** `test_players_profileHeaderControls_areHittable` **passed**. An inspector section header's borderless controls *are* AX-visible on macOS — M1's sidebar finding does **not** transfer, the rename pencil and actions menu need no menu-bar remedy, and the three UITests that were queued behind the answer are not needed. The experiment was written to make a failure name its own cause in one line; it answered the other way, which is the better outcome and the same one line.
...
D-numbers are sequential and never reused. Next free number: **D42′**.
