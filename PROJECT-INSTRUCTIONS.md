...
Revision 1 August 2026 — **M8 delivered whole**, both items, the first milestone in this document's life to land in one pass. Base named explicitly, per the working agreement: this is the 31 July third-of-day revision (D44′) plus this pass's deltas — **D45′, D46′, seven commits, six new source files, and one milestone moving to Landed**. Everything the earlier revisions recorded stands except where corrected below. Written by editing the file at its anchors.

Two decisions minted at recording time: **D45′** (inspector sections collapse, on one persisted key) and **D46′** (the evaluation graph opens in its own window). Both were roadmap items with their decisions pre-flagged; both turned out to have a *prerequisite* nobody had written down, which is this pass's through-line.

Base for this delta is **`f64b8d4`**, and **the tree was not clean**: D44′ was sitting in it, uncommitted, while the revision above described it as delivered. The streak resumed at one. `git status` caught it as the first command, for the sixth time out of seven passes. The rule has now been written down four separate ways and prevented nothing; running the command has caught it every time. That is the entire lesson and it is not going to improve.

**This pass produced code, and a lot of it: 211 → 217 sources across seven commits, ⌘U green after each of the two review points.** M8's two features were the smaller half. The larger half was a prerequisite the milestone could not have known it had.

**The finding that reshaped the milestone.** M8 item 2 said "every `InspectorSectionHeader` grows a show/hide chevron", which reads as a complete instruction and is not one: **eight of the app's fifteen inspector section headers went through that type and seven did not**. Building the feature as written would have made 8 of 15 sections collapsible with no rule a reader could perceive — Board's Game and Opening folding while its Evaluation and Moves could not — which is precisely the divergence D26′ exists to prevent, minted by a milestone whose stated constraint was not breaking D26′. Item 1 had the same blocker: the Evaluation header it wanted to hang a magnifier on was a raw `Text` in both inspectors.

**This is a fourth species of unchecked claim, and it is the one this document is least equipped to catch.** D42′ generalised the standing rule from "the tree is clean" to **the file is there**; D43′ extended it to **the number is current**; D44′ to **the rule is real**. This pass adds **the set is complete**. The previous three were sentences that were *false*. "Every `InspectorSectionHeader`" was entirely *true* — it just quantified over a smaller set than the reader of it (me, and the milestone's author) assumed, and nothing about a true statement ever fails. A count is the cheapest possible check and nobody counts, because a sentence that says "every" already sounds like someone did.

Next free number: **D47′**.
...
Tree at delivery: **`caae1d7`**, clean, with the seven commits of this pass on `f64b8d4` — D44′'s two, M8's prep, the grep-noise sweep, D45′'s two, D46′, and two build fixes. Checked with `git ls-files` and `git status`, not assumed. **217 sources (135 app, 81 unit-test, 1 UITest)**, up from 211.
